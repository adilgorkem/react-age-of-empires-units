...
import UnitsAges from './Units/UnitsAges';
...
const Units = () => {
  ...
  const [woodRange, setWoodRange] = useState(200);
  const [foodRange, setFoodRange] = useState(200);
  const [goldRange, setGoldRange] = useState(200);
  const [unitDetail, setUnitDetail] = useState(0);
  ...
  const AgesFilterHandler = submittedAge => {
    setFilteredAge(submittedAge)
  }
  ...
  return (
    <>
    {unitDetail ? <UnitDetail properties={data.filter(item => item.id === unitDetail)}/> : <div>
      <UnitsAges selected={filteredAge} onChangeFilter={AgesFilterHandler} />
      <div style={{marginBottom: '50px', marginTop: '50px', display:'flex', flexDirection:'column'}} onChange={handleChange}>
        <div className='costs'>
        <label htmlFor="wood">Wood</label>
        <input type="range" id='wood' min={0} max={200}></input>
        </div>
        <div className='costs'>
        <label htmlFor="food">Food</label>
        <input type="range" id='food' min={0} max={200}></input>
        </div>
        <div className='costs'>
        <label htmlFor="gold">Gold</label>
        <input type="range" id='gold' min={0} max={200}></input>
        </div>
      </div>
      <div>
        <table>
          ...
          .filter(unit => {
            if (filteredAge !== 'All') {
              return unit.age === filteredAge
            }
            return true
          })
          .filter(unit => {
            if(unit.cost?.Food) {
              return unit.cost?.Food < foodRange
            }
            return true
          })
          .filter(unit => {
            if(unit.cost?.Wood) {
              return unit.cost?.Wood < woodRange
            }
            return true
          })
          .filter(unit => {
            if(unit.cost?.Gold) {
              return unit.cost?.Gold < goldRange
            }
            return true
          })
          .map((unit) => (
            <UnitsList key={unit.id} id={unit.id} name={unit.name} age={unit.age} cost={unit.cost} handleClick={handleClick} />))}
        </table>
      </div>
      <div>
        <br></br>
